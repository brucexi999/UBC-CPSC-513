		...
		Only prints the clauses if print is nonzero. */
{
	...
	switch (c->wires[i]->type) {
        ...
        // If any input is 1, output must be 0
        for (j = 0; j < c->wires[i]->fanincount; j++) {
            print("-%d -%d 0\n",
                c->wires[c->wires[i]->fanins[j]]->var_num,
                c->wires[i]->var_num);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    /* FIX THIS!!! */
    /* YOUR CODE GOES HERE. */
    // Generate a clause that includes all XOR variables, if at least 1 literal is true, then the clause must be true
    for (i = 0; i < circuit1->outputcount; i++) {
        int xor_var_num = circuit1->wirecount + circuit2->wirecount + i + 1;
        printf("%d ", xor_var_num);  // Add all XOR variable
    }
    printf("0\n");
    return 0;
}
